...
import ProjetCPOA.RobotHostile;
import ProjetCPOA.Robot;
...
public class Simulator {
	...
	public synchronized void simulateOneStep() {
		...
		if (counter % 100000 == 0) {
			...
			System.out.println("Memory heap total: " + Runtime.getRuntime().totalMemory() / 1024 + "k  max: "
					+ Runtime.getRuntime().maxMemory() / 1024 + "k  free: " + Runtime.getRuntime().freeMemory() / 1024
					+ "k");
		}
		counter++;
		...
		for (int i = 0; i < nagents; i++) {
			...
			//if ( a instanceof Robot && robotStart == false ) continue;
		...
		for (int i = 0; i < nagents; i++) {
			...
			a.clearVeryNear();
			//if ( a instanceof Robot && robotStart == false ) continue;
			if (a instanceof RobotHostile)
				a.setMotorsAcceleration(dt);
			else {
				a.setMotorsAcceleration(dt, direction);
	...
	public void setRobotStart(boolean robotStart) {
		this.robotStart = robotStart;
	}
